...
model: haiku
allowed-tools:
  - Bash(python3 /Users/changhwan/workspace/tech_blog/.claude/skills/blog:new-post/scripts/new_post.py *)
  - Read
  - Write
---
# blog:new-post
...
> **한글 제목 주의**: 한글 제목은 slug 자동 생성 시 `new-post`가 될 수 있다.
> 한글 제목 사용 시 `--slug`를 반드시 영문으로 지정하도록 안내한다.

### Step 2 — dry-run 확인

```bash
python3 /Users/changhwan/workspace/tech_blog/.claude/skills/blog:new-post/scripts/new_post.py \
  --title "제목" \
  --slug "my-post-slug" \
...
사용자 확인 후 (또는 바로 진행 요청 시) `--dry-run` 없이 실행한다.
...
- thumbnail 없이 빌드하면 이미지 관련 에러 발생 가능 — 임시 이미지를 먼저 넣거나 frontmatter의 thumbnail 필드를 제거하고 진행
